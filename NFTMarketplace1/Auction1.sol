...
/// @notice This contract can be used for auctioning any NFTs, and accepts any BEP20 token as payment
contract NFTAuction {
    ...
    struct Auction {
        ...
        address BEP20Token; // The seller can specify an BEP20 token that can be used to bid or purchase the NFT.
    ...
    // EVENTS 
    event NftAuctionCreated(address nftContractAddress,uint256 tokenId,address nftSeller,address bep20Token,uint128 minPrice,uint128 buyNowPrice,uint32 auctionBidPeriod,uint32 bidIncreasePercentage,address[] feeRecipients,uint32[] feePercentages);
    event SaleCreated(address nftContractAddress,uint256 tokenId,address nftSeller,address bep20Token,uint128 buyNowPrice,address whitelistedBuyer,address[] feeRecipients,uint32[] feePercentages);
    event BidMade(address nftContractAddress,uint256 tokenId,address bidder,uint256 ethAmount,address bep20Token,uint256 tokenAmount);
    event AuctionPeriodUpdated(address nftContractAddress,uint256 tokenId,uint64 auctionEndPeriod);
    event NFTTransferredAndSellerPaid(address nftContractAddress,uint256 tokenId,address nftSeller,uint128 nftHighestBid,address nftHighestBidder,address nftRecipient);
    ...
    modifier isAuctionNotStartedByOwner(address _nftContractAddress,uint256 _tokenId) {
        require(
            nftContractAuctions[_nftContractAddress][_tokenId].nftSeller != msg.sender,"Auction already started by owner");
        if (nftContractAuctions[_nftContractAddress][_tokenId].nftSeller != address(0)) {
            require(msg.sender == IBEP721(_nftContractAddress).ownerOf(_tokenId),"Sender doesn't own NFT");
    ...
    modifier minPriceDoesNotExceedLimit(uint128 _buyNowPrice,uint128 _minPrice) {
        require(_buyNowPrice == 0 || _getPortionOfBid(_buyNowPrice, maximumMinPricePercentage) >= _minPrice,"MinPrice > 80% of buyNowPrice");
        _;
    }
    ...
    // The bid amount was either equal the buyNowPrice or it must be higher than the previous bid by the specified bid increase percentage.
    ...
    modifier onlyApplicableBuyer(address _nftContractAddress,uint256 _tokenId) {
        require(
            !_isWhitelistedSale(_nftContractAddress, _tokenId) || nftContractAuctions[_nftContractAddress][_tokenId].whitelistedBuyer == msg.sender,"Only the whitelisted buyer");
        _;
    }
    ...
    // Payment is accepted if the payment is made in the BEP20 token or ETH specified by the seller.
    // Early bids on NFTs not yet up for auction must be made in ETH.
    modifier paymentAccepted(address _nftContractAddress,uint256 _tokenId,address _bep20Token,uint128 _tokenAmount) {
        require(_isPaymentAccepted(_nftContractAddress,_tokenId, _bep20Token, _tokenAmount), "Bid to be in specified BEP20/Eth");
        _;
    }
    ...
    modifier correctFeeRecipientsAndPercentages(uint256 _recipientsLength,uint256 _percentagesLength) {
    ...
    // Check if a bid has been made. This is applicable in the early bid scenario to ensure that if an auction is 
    // created after an early bid, the auction begins appropriately or is settled if the buy now price is met.
    ...
    function _isASale(address _nftContractAddress, uint256 _tokenId)internal view returns (bool){
        return (nftContractAuctions[_nftContractAddress][_tokenId].buyNowPrice >0 && nftContractAuctions[_nftContractAddress][_tokenId].minPrice == 0);
    }
    ...
    function _isHighestBidderAllowedToPurchaseNFT(address _nftContractAddress,uint256 _tokenId) internal view returns (bool) {
        return (!_isWhitelistedSale(_nftContractAddress, _tokenId)) || _isHighestBidderWhitelisted(_nftContractAddress, _tokenId);
    }
     ...
     *  --------> Cannot bid with ETH & an BEP20 Token together in any circumstance<------
     * (2) Auction not created - only ETH accepted (cannot early bid with an BEP20 Token
     * (3) Cannot make a zero bid (no ETH or Token amount)
     */
    function _isPaymentAccepted(address _nftContractAddress,uint256 _tokenId,address _bidBEP20Token,uint128 _tokenAmount) internal view returns (bool) {
        address auctionBEP20Token = nftContractAuctions[_nftContractAddress][_tokenId].BEP20Token;
        if (_isBEP20Auction(auctionBEP20Token)) {
            return msg.value == 0 && auctionBEP20Token == _bidBEP20Token && _tokenAmount > 0;
        } else {
            return msg.value != 0 && _bidBEP20Token == address(0) && _tokenAmount == 0;
        }
    }
    function _isBEP20Auction(address _auctionBEP20Token)internal pure returns (bool){
        return _auctionBEP20Token != address(0);
    }
    ...
    // These functions check if the applicable auction parameter has been set by the NFT seller. If not, return the default value. 
    ...
    function _transferNftToAuctionContract(address _nftContractAddress,uint256 _tokenId) internal {
        ...
        if (IBEP721(_nftContractAddress).ownerOf(_tokenId) == _nftSeller) {
            IBEP721(_nftContractAddress).transferFrom(_nftSeller,address(this),_tokenId);
            require(IBEP721(_nftContractAddress).ownerOf(_tokenId) == address(this),"nft transfer failed");
        } else {
            require(IBEP721(_nftContractAddress).ownerOf(_tokenId) == address(this),"Seller doesn't own NFT");
        }
    }
    // AUCTION CREATION 
    /*
     * Setup parameters applicable to all auctions and whitelised sales:
     * -> BEP20 Token for payment (if specified by the seller) : _bep20Token
     ...
     */
    function _setupAuction(address _nftContractAddress,uint256 _tokenId,address _bep20Token,uint128 _minPrice,uint128 _buyNowPrice,
    address[] memory _feeRecipients,uint32[] memory _feePercentages)internal minPriceDoesNotExceedLimit(_buyNowPrice, _minPrice)
        correctFeeRecipientsAndPercentages(_feeRecipients.length,_feePercentages.length) isFeePercentagesLessThanMaximum(_feePercentages){
        if (_bep20Token != address(0)) {
            nftContractAuctions[_nftContractAddress][_tokenId].BEP20Token = _bep20Token;
        }
    ...
    function _createNewNftAuction(address _nftContractAddress,uint256 _tokenId,address _bep20Token,uint128 _minPrice,
        uint128 _buyNowPrice,address[] memory _feeRecipients,uint32[] memory _feePercentages) internal {
        // Sending the NFT to this contract
        _setupAuction(_nftContractAddress,_tokenId,_bep20Token,_minPrice,_buyNowPrice,_feeRecipients,_feePercentages);
        emit NftAuctionCreated(_nftContractAddress,_tokenId,msg.sender,_bep20Token,_minPrice,_buyNowPrice,_getAuctionBidPeriod(_nftContractAddress, 
        _tokenId),_getBidIncreasePercentage(_nftContractAddress, _tokenId),_feeRecipients,_feePercentages);
    ...
    function createDefaultNftAuction(address _nftContractAddress,uint256 _tokenId,address _bep20Token,uint128 _minPrice,
        uint128 _buyNowPrice,address[] memory _feeRecipients, uint32[] memory _feePercentages) external
        isAuctionNotStartedByOwner(_nftContractAddress, _tokenId) priceGreaterThanZero(_minPrice){
        _createNewNftAuction(_nftContractAddress,_tokenId,_bep20Token, _minPrice,_buyNowPrice,_feeRecipients,_feePercentages);
    }
    function createNewNftAuction(address _nftContractAddress,uint256 _tokenId,address _bep20Token,uint128 _minPrice,uint128 _buyNowPrice,
        uint32 _auctionBidPeriod, //this is the time that the auction lasts until another bid occurs
        uint32 _bidIncreasePercentage,address[] memory _feeRecipients,uint32[] memory _feePercentages)external
        isAuctionNotStartedByOwner(_nftContractAddress, _tokenId) priceGreaterThanZero(_minPrice) increasePercentageAboveMinimum(_bidIncreasePercentage){
    ...
    // SALES
    // Allows for a standard sale mechanism where the NFT seller can can select an address to be whitelisted. 
    // This address is thenallowed to make a bid on the NFT. No other address can bid on the NFT
    function _setupSale(address _nftContractAddress,uint256 _tokenId,address _bep20Token,uint128 _buyNowPrice,address _whitelistedBuyer,address[] memory _feeRecipients,uint32[] memory _feePercentages)
        internal
        correctFeeRecipientsAndPercentages(_feeRecipients.length,_feePercentages.length) isFeePercentagesLessThanMaximum(_feePercentages){
        if (_bep20Token != address(0)) {
            nftContractAuctions[_nftContractAddress][_tokenId].BEP20Token = _bep20Token;
    ...
    function createSale(address _nftContractAddress,uint256 _tokenId,address _bep20Token,uint128 _buyNowPrice,address _whitelistedBuyer,address[] memory _feeRecipients,uint32[] memory _feePercentages)
        external isAuctionNotStartedByOwner(_nftContractAddress, _tokenId) priceGreaterThanZero(_buyNowPrice){
        //min price = 0
        _setupSale(_nftContractAddress,_tokenId,_bep20Token,_buyNowPrice, _whitelistedBuyer,_feeRecipients,_feePercentages);
        emit SaleCreated(_nftContractAddress,_tokenId,msg.sender, _bep20Token,_buyNowPrice,_whitelistedBuyer, _feeRecipients,_feePercentages);
    ...
    // BID FUNCTIONS 
    // Make bids with ETH or an BEP20 Token specified by the NFT seller.Additionally, a buyer can pay the asking price to conclude a sale of an NFT.                                                     
    function _makeBid(address _nftContractAddress,uint256 _tokenId,address _bep20Token,uint128 _tokenAmount)internal notNftSeller(_nftContractAddress, _tokenId) 
    paymentAccepted(_nftContractAddress,_tokenId,_bep20Token,_tokenAmount)bidAmountMeetsBidRequirements(_nftContractAddress,_tokenId,_tokenAmount){
        _reversePreviousBidAndUpdateHighestBid(_nftContractAddress,_tokenId,_tokenAmount);
        emit BidMade(_nftContractAddress,_tokenId, msg.sender,msg.value, _bep20Token,_tokenAmount);
        _updateOngoingAuction(_nftContractAddress, _tokenId);
    }
    function makeBid(address _nftContractAddress,uint256 _tokenId,address _bep20Token, uint128 _tokenAmount)external payable 
    auctionOngoing(_nftContractAddress, _tokenId) onlyApplicableBuyer(_nftContractAddress, _tokenId){
        _makeBid(_nftContractAddress, _tokenId, _bep20Token, _tokenAmount);
    }
    function makeCustomBid(address _nftContractAddress,uint256 _tokenId,address _bep20Token,uint128 _tokenAmount,address _nftRecipient)
        external payable auctionOngoing(_nftContractAddress, _tokenId) notZeroAddress(_nftRecipient) onlyApplicableBuyer(_nftContractAddress, _tokenId){
        nftContractAuctions[_nftContractAddress][_tokenId].nftRecipient = _nftRecipient;
        _makeBid(_nftContractAddress, _tokenId, _bep20Token, _tokenAmount);
    }
    // UPDATE AUCTION 
    // Settle an auction or sale if the buyNowPrice is met or setauction period to begin if the minimum price has been met.
    ...
    function _updateAuctionEnd(address _nftContractAddress, uint256 _tokenId)internal {
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].auctionEnd =_getAuctionBidPeriod(_nftContractAddress, _tokenId) +
        uint64(block.timestamp);
        emit AuctionPeriodUpdated(_nftContractAddress,_tokenId,nftContractAuctions[_nftContractAddress][_tokenId].auctionEnd);
    }
    // RESET FUNCTIONS
    // Reset all auction related parameters for an NFT.This effectively removes an EFT as an item up for auction
    function _resetAuction(address _nftContractAddress, uint256 _tokenId) internal{
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].BEP20Token = address(0);
    }
    // Reset all bid related parameters for an NFT.This effectively sets an NFT as having no active bids
    ...
    // UPDATE BIDS 
    // Internal functions that update bid parameters and reverse bidsto ensure contract only holds the highest bid.
    function _updateHighestBid(address _nftContractAddress,uint256 _tokenId,uint128 _tokenAmount) internal {
        address auctionBEP20Token = nftContractAuctions[_nftContractAddress][_tokenId].BEP20Token;
        if (_isBEP20Auction(auctionBEP20Token)) {
            IBEP20(auctionBEP20Token).transferFrom(msg.sender,address(this),_tokenAmount);
    ...
    function _transferNftAndPaySeller(address _nftContractAddress,uint256 _tokenId) internal {
        ...
        IBEP721(_nftContractAddress).transferFrom(address(this),_nftRecipient, _tokenId);
        _resetAuction(_nftContractAddress, _tokenId);
    ...
    function _payout(address _nftContractAddress,uint256 _tokenId,address _recipient,uint256 _amount) internal {
        address auctionBEP20Token = nftContractAuctions[_nftContractAddress][_tokenId].BEP20Token;
        if (_isBEP20Auction(auctionBEP20Token)) {
            IBEP20(auctionBEP20Token).transfer(_recipient, _amount);
        } else {
    ...
    function withdrawAuction(address _nftContractAddress, uint256 _tokenId)external{
        ...
        require(IBEP721(_nftContractAddress).ownerOf(_tokenId) == msg.sender,"Not NFT owner");
        _resetAuction(_nftContractAddress, _tokenId);
    ...
    function updateMinimumPrice(address _nftContractAddress,uint256 _tokenId,uint128 _newMinPrice)external onlyNftSeller(_nftContractAddress, _tokenId)
        minimumBidNotMade(_nftContractAddress, _tokenId) isNotASale(_nftContractAddress, _tokenId) priceGreaterThanZero(_newMinPrice)
    ...
    function updateBuyNowPrice(address _nftContractAddress,uint256 _tokenId,uint128 _newBuyNowPrice)external onlyNftSeller(_nftContractAddress, _tokenId)
        priceGreaterThanZero(_newBuyNowPrice) minPriceDoesNotExceedLimit(_newBuyNowPrice,nftContractAuctions[_nftContractAddress][_tokenId].minPrice){
