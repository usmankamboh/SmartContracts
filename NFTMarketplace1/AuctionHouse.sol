...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    address public sss;
    // / The address of the WBNB contract, so that any BNB transferred can be handled as an ERC-20
    ...
    constructor(address _sss, address _wbnb){
        require(IERC165(_sss).supportsInterface(interfaceId),"Doesn't support NFT interface");
        sss = _sss;
        wbnbAddress = _wbnb;
    ...
    function createAuction(uint256 tokenId,address tokenContract,uint256 duration,uint256 reservePrice,address payable curator,
        ...
        require(IERC165(tokenContract).supportsInterface(interfaceId),"tokenContract does not support ERC721 interface");
    ...
    function _handlesssAuctionSettlement(uint256 auctionId) internal returns (bool, uint256) {
        ...
        IERC20(currency).approve(IMediaExtended(sss).marketContract(), bid.amount);
