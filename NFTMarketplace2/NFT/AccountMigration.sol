...
import "./IERC1271.sol";
abstract contract AccountMigration is OperatorRole {
  ...
  modifier onlyAuthorizedAccountMigration(
    ...
    require(_isOperator(), "AccountMigration: Caller is not an operator");
    require(originalAddress != newAddress, "AccountMigration: Cannot migrate to the same account");
    bytes32 hash = _toEthSignedMessage(
      abi.encodePacked("I authorize Smart to migrate my account to ", _toAsciiString(newAddress))
    );
    require(
