...
contract UniswapV2Locker is Ownable, ReentrancyGuard {
  ...
  struct FeeStruct {
    ...
    uint256 referralPBEPent; // fee for referrals
  ...
  function setFees(uint256 _referralPBEPent, uint256 _referralDiscount, uint256 _bnbFee, uint256 _secondaryTokenFee, uint256 _secondaryTokenDiscount, uint256 _liquidityFee) public onlyOwner {
    gFees.referralPBEPent = _referralPBEPent;
    ...
    if (!feeWhitelist.contains(msg.sender)) {
      if (_fee_in_bnb) { // charge fee in bnb
        ...
        if (bnbFee != 0 && _referral != address(0)) { // referral fee
          uint256 referralFee = devFee.mul(gFees.referralPBEPent).div(1000);
        ...
        if (gFees.referralPBEPent != 0 && _referral != address(0)) { // referral fee
          uint256 referralFee = burnFee.mul(gFees.referralPBEPent).div(1000);
